...
import { getGoogleOauthUrl } from "@/app/(auth)/authenticate/auth.action";
    ...
    <div className=''>
      <Button
        onClick={async () => {
          ...
          console.log("🚀 ~ onClick={ ~ url:", response);
          if (response.success && response.url) {
            openPopup(response.url, "Google OAuth", 600, 700);
          } else {
...
const openPopup = (url: string, title: string, w: number, h: number) => {
